...
//    qsort_dbl_key_int_val(eigval, ev_idx, 0, nbf);
    ...
    memcpy(tmp_mat, S_mat, DBL_MSIZE * mat_size);
    LAPACKE_dsyev(LAPACK_ROW_MAJOR, 'V', 'U', nbf, tmp_mat, nbf, eigval);
    for(int i=0;i<nbf;i++)
    {
        printf("%f ",eigval[i]);
...
void TinyDFT_MP2processtest(TinyDFT_p TinyDFT)
{
    int    nbf       = TinyDFT->nbf;
    int    n_occ     = TinyDFT->n_occ;
    int    n_vir     = TinyDFT->n_vir;
...
//    double *orbitenergy_array = TinyDFT->orbitenergy_array;
    ...
    printf("Now the sorting array is:\n");
    for (int i=0;i<nbf;i++)
        printf("%d, ",ev_idx[i]);

    printf("The sorted eigenvalues are:\n");
    for (int i=0;i<nbf;i++)
        printf("%f, ", eigval[ev_idx[i]]);
    // D = C_occ * C_occ^T
    cblas_dgemm(
        CblasRowMajor, CblasNoTrans, CblasTrans, nbf, nbf, n_occ, 
        1.0, Cocc_mat, n_occ, Cocc_mat, n_occ, 0.0, D_mat, nbf
