...
int main(int argc, char **argv)
{
    ...
    TinyDFT_build_MP2info_eig(TinyDFT,TinyDFT->F_mat,TinyDFT->X_mat,TinyDFT->D_mat,TinyDFT->Cocc_mat,TinyDFT->DC_mat, TinyDFT->Cvir_mat, TinyDFT->orbitenergy_array);
    printf("---------------Now test density matrix information---------------");
    printf("Firstly, print out the molecular orbital energy in ascending order");
    for(int i=0;i<nbf;i++)
    {
        printf("%f ",TinyDFT->orbitenergy_array[i]);
    }
    printf("In this test, the number of nonzero eigenvalues of density matrix should be equal to n_occ");
    memcpy(TinyDFT->tmp_mat, TinyDFT->D_mat, DBL_MSIZE * TinyDFT->mat_size);
    LAPACKE_dsyev(LAPACK_ROW_MAJOR, 'V', 'U', nbf, TinyDFT->tmp_mat, nbf, TinyDFT->eigval);
    int param=0;
    for(int i=0;i<TinyDFT->nbf;i++)
    {
//        printf("%f \n", TinyDFT->eigval[i]);
        if(TinyDFT->eigval[i]>0.01)
        {
            param +=1 ;
        }
    }
    printf("The number of nonzero eigenvalues in D is %d \n",param);
    printf("n_occ = %d", TinyDFT->n_occ);
    memcpy(TinyDFT->tmp_mat, TinyDFT->DC_mat, DBL_MSIZE * TinyDFT-> mat_size);
    LAPACKE_dsyev(LAPACK_ROW_MAJOR, 'V', 'U', nbf, TinyDFT->tmp_mat, nbf, TinyDFT->eigval);
    param=0;
    for(int i=0;i<TinyDFT->nbf;i++)
    {
//        printf("%f \n", TinyDFT->eigval[i]);
    ...
    printf("The number of nonzero eigenvalues in DC is %d \n",param);
    printf("n_vir = %d", TinyDFT->n_vir);
    printf("--------------------------------------------------");
    printf("D_mat+DC_mat should be S^-1, so we test the Frobenius norm of (D+DC)*S and it should be nbf\n");
    for(int i=0;i<TinyDFT->nbf;i++)
        for(int j=0;j<TinyDFT->nbf;j++)
        {
            TinyDFT->tmp_mat[i*nbf+j]=TinyDFT->D_mat[i*nbf+j]+TinyDFT->DC_mat[i*nbf+j];
        }
    cblas_dgemm(
        CblasRowMajor, CblasNoTrans, CblasNoTrans, nbf, nbf, nbf, 
        1.0, TinyDFT->S_mat, nbf, TinyDFT->tmp_mat, nbf, 0.0, TinyDFT->D_mat, nbf);
    
    double norms=0;
    for(int i=0;i<TinyDFT->nbf;i++)
        for(int j=0;j<TinyDFT->nbf;j++)
        {
            norms=norms+TinyDFT->D_mat[i*nbf+j]*TinyDFT->D_mat[i*nbf+j];
        //    printf("%f ", TinyDFT->D_mat[i*nbf+j]);
        }
    ...
    cblas_dgemm(
        CblasRowMajor, CblasNoTrans, CblasNoTrans, nbf, nbf, nbf, 
        1.0, TinyDFT->tmp_mat, nbf, TinyDFT->X_mat, nbf, 0.0, TinyDFT->S_mat, nbf);

    norms=0;
    for(int i=0;i<TinyDFT->nbf;i++)
        for(int j=0;j<TinyDFT->nbf;j++)
        {
            norms=norms+TinyDFT->S_mat[i*nbf+j]*TinyDFT->S_mat[i*nbf+j];
        //    printf("%f ", TinyDFT->D_mat[i*nbf+j]);
        }
